...
export default function ExaminationPrescriptionsPage() {
  ...
  const { data: prescriptions = [], isLoading } = useQuery({
    queryKey: queryKeys.prescriptions.byExamination(examId),
    queryFn: () => getExaminationPrescriptions(examId),
    onError: (err: Error) => {
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-3xl font-bold">Prescriptions for Examination #{examinationId}</h1>
        <div className="flex gap-2">
          <Button asChild>
            <Link href={`/prescriptions/examination/${examinationId}/new`}>
